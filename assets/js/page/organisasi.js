var leadPeriode = document.querySelector('#leadPeriode');
leadPeriode.innerHTML = "Periode 2023 - 2026";
// Data Organisasi
...
for (let s = 0; s < cardSyuroPembina.length; s++) {
    let isiSyuroPembina = "";
    isiSyuroPembina += `<div class="card-header shadow-sm bg-success text-light font-weight-bold text-center text-uppercase h5 p-2">${syuroPembina[s].jabatan}</div><div class="card-body d-flex flex-column justify-content-center align-items-center p-1">`;
...
pengurusHarian.forEach((ph) => {
    isiPengurusHarian += `<div class="m-2">
                            <p class="font-weight-bold text-success m-0">${ph.jabatan}</p>
...
bidangBidang.forEach((b) => {
    isiBidangBidang += `<div class="card border border-success flex-fill shadow-sm m-2">
                            <div class="card-header shadow-sm bg-success text-center text-light font-weight-bold text-uppercase h5 p-2">Bidang<br>${b.bidang}</div>
                            <div class="card-body d-flex flex-column text-center p-1">
                                <div class="m-0">
                                    <p class="font-weight-bold text-success m-0">Ketua</p>
                                    <p class="card-text m-0">${b.ketua}</p>
                                </div>
                                <div class="m-2"><p class="font-weight-bold text-success m-0">Anggota</p>`;
